...
| **S3** | SA-3D RoPE + multiseg | rope cos/sin ≤1e-4 vs oracle (grids (3,4,4)/(2,6,8), source_ids 0/1/3); `forwardMultiseg([], target)` ≡ plain t2v ≤1e-3 |
| **S4** | r2v (APG) → v2v → rv2v | per-step guided-x0 vs oracle dump ≤1e-3 (first 3 steps, injected); e2e subject-preservation goldens |
| **S5** | Streaming decode | **bit-identical** to whole-seq decode on CPU stream (the oracle's own gate), flat memory across 1/2/3/5-chunk params |
